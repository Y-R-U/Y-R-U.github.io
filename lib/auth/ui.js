...
const PLAYTIME_KEY = "br8t_playms";        // visible time, ms — NOT wall clock
const PESTER_GAP_MS = 7 * 60 * 1000;       // played, between one pester and the next
...
export function matchCompleted(gameId = cfg.gameId) {
  ...
  if (cfg.nudge === "callout") { offerSignIn(); return; }
  if (s.done) return;
...
function pesterState() {
  try { return JSON.parse(localStorage.getItem(PESTER_KEY)) || { off: false, last: 0 }; }
  catch (e) { return { off: false, last: 0 }; }
}
function setPesterState(s) {
  try { localStorage.setItem(PESTER_KEY, JSON.stringify(s)); } catch (e) { /* private mode */ }
}

// Play time, not wall clock — a tab left open overnight has not been played.
function playMs() {
  const n = Number(localStorage.getItem(PLAYTIME_KEY));
...
function startPlayClock() {
  ...
  setInterval(() => {
    ...
    if (dt > 5000) return;              // throttled in a background tab
    try { localStorage.setItem(PLAYTIME_KEY, String(playMs() + dt)); } catch (e) { /* private mode */ }
  }, 2000);
...
export function offerSignIn() {
  ...
  if (auth.user && !auth.user.anon) return false;          // already signed in
  const s = pesterState();
  ...
  const played = playMs();
  if (s.last && played - s.last < PESTER_GAP_MS) return false;
  s.last = played;
  setPesterState(s);
  showPester();
  return true;
...
export function mountAccount(options = {}) {
  ...
  renderFab();
  if (options.nudge === "callout") startPlayClock();
