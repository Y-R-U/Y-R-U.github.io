...
import { mountAccount, matchCompleted } from "./ui.js";
...
export function syncLocalKeys(options) {
  const { gameId, keys, describe, onAdopt } = options;
  ...
  return { snapshot, push: () => slot.flush(), matchCompleted: () => matchCompleted(gameId) };
}

export { matchCompleted, auth, cloud };
