...
async function loadSDK() {
  ...
  // Spell the persistence out rather than trusting getAuth()'s defaults. The
  // list is tried in order, so a browser with IndexedDB walled off (private
  // windows, hardened privacy settings) still keeps the session in
  // localStorage instead of silently falling back to in-memory — which logs
  // the player out on every refresh.
  //
  // initializeAuth() also means popupRedirectResolver has to be passed
  // explicitly; without it signInWithPopup throws auth/argument-error.
  const auth = a.initializeAuth(application, {
...
// Re-read the live user and push it out. Called after every operation that can
// change who we are, so the UI never depends on a listener having fired.
function refresh(user) {
  current = normalise(user || (fb && fb.auth.currentUser));
  emit();
  return current;
}
...
function init() {
  ...
  readyP = (async () => {
    ...
    // onIdTokenChanged, NOT onAuthStateChanged: linking a provider to the
    // anonymous account mutates the current user instead of swapping it, so
    // onAuthStateChanged stays silent and the avatar would keep showing a guest
    // until the next reload. The token change fires either way.
    await new Promise(res => {
      let first = true;
      mod.onIdTokenChanged(a, u => {
        current = normalise(u);
        emit();
    ...
    // Be certain persistence has finished restoring before concluding that
    // nobody is signed in — creating an anonymous user over a real session
    // would silently sign the player out on every reload.
    try { await a.authStateReady(); } catch (e) { /* older SDKs: observer above already waited */ }
    if (!a.currentUser) {
...
export class AccountConflict extends Error {
  ...
  async adopt() {
    ...
    // We deliberately do NOT delete the orphaned anonymous user here. Deleting
    // a User object belonging to this same auth instance runs sign-out
    // bookkeeping on it, which can land in persistence AFTER the real session
    // was written — leaving the player signed in until they reload, then
    // mysteriously signed out. A stray anonymous record is free; every visitor
    // creates one anyway. Not worth risking the session to tidy it up.

    // Force a token write so the restored session is definitely on disk before
    // the player has a chance to reload.
    try { await res.user.getIdToken(true); } catch (e) { /* non-fatal */ }
    return refresh(res.user);
  }
}
...
export async function signOut() {
  const { mod, auth: a } = await loadSDK();
  await mod.signOut(a);
  const res = await mod.signInAnonymously(a);
  return refresh(res.user);
}
...
export const auth = {
  // Resolves to the user as it is NOW, not the snapshot from first boot — the
  // player may have signed in since.
  async ready() { await init(); return current; },
