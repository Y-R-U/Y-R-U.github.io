...
const PROJECTS = [
  ...
  { name: "KITEHAWK",        path: "/gms/2d/kitehawk/",  screenshot: "kitehawk",       type: "game", wip: true,
    desc: "A painterly biplane game about altitude. You fly one thumb — hold and slide, the throttle looks after itself, the guns fire in a cone off the nose — and everything else is where you are in the column of sky. Six bands from the Mud at the deck to the Blue at the top, and a fight is won or lost on which of them you are in when it starts. Supply crates come down under parachutes and they are the whole economy: fly through one to catch it, or cut its canopy and let it fall, which is what you do to a crate you were never going to reach. Real aerodynamics underneath — a stall you can feel, a dive that takes the whole screen to pull out of, structural stress that will take a wing off if you ask too much. Vanilla JS and WebGL2, no build step, no CDN, mobile-first in landscape. In active build: it flies, it fights, the crates come down and a level plays start to finish. The hundred levels, the story and the art pass are still ahead of it.",
