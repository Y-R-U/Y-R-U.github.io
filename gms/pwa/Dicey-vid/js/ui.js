...
const UI = {
    ...
    updateCardsStrip(gameState, playerIndex) {
        ...
        if (owned.length === 0) {
            ...
            scroll.innerHTML = '';
            this.refreshBoardLayoutAfterCardsChange();
            return;
        }
    ...
    refreshBoardLayoutAfterCardsChange() {
        if (typeof BoardRenderer === 'undefined' || !BoardRenderer.resize) return;
        requestAnimationFrame(() => {
            BoardRenderer.resize();
            setTimeout(() => BoardRenderer.resize(), 230);
        });
    },
