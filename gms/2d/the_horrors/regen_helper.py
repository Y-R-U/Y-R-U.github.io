...
IMAGE_WIDTH = 768
IMAGE_HEIGHT = 1280
# js/variants.js is written by gen_monsters.py and rewritten here whenever a
# reviewer picks a variant or tunes a zoom — keep the header byte-identical in
# both writers so the file does not churn.
VARIANTS_HEADER = (
...
def monster_ref_file(project, monster_id):
    for name in (f"monster_{monster_id}.jpg", f"monster_{monster_id}.png"):
        if os.path.exists(os.path.join(project.ref_dir, name)):
            return name
    return f"monster_{monster_id}.jpg"


def load_monsters(root):
    """monsters.json — the per-creature art brief gen_monsters.py works from."""
    path = os.path.join(root, "monsters.json")
    try:
        with open(path, "r", encoding="utf-8") as handle:
            config = json.load(handle)
    except (FileNotFoundError, json.JSONDecodeError):
        return {}
    return {monster["id"]: monster for monster in config.get("monsters", [])}


def apply_monster_prompts(project):
    """Point the monster event rows at the generated start frames and the
    motion-only video prompts. The prompts in gen_event_videos.py describe the
    creature's appearance, which is now the still's job — asking LTX to invent
    the creature as well as animate it is what made those clips inconsistent."""
    for file_name, transition in project.event_transitions.items():
        monster_id, kind = monster_id_from_transition(file_name)
        ...
        if video_prompt:
            ...
            # monsters.json is the source of truth for these — a promptText
            # saved in .debug_transition_metadata.json by the old
            # describe-the-creature pipeline must not shadow it.
            transition["promptFromConfig"] = True
...
def build_image_meta(project):
    ...
    for file_name, transition in project.event_transitions.items():
        ...
        meta[file_name] = {
            ...
            "monsterRefFile": monster_ref_file(project, monster_id),
...
class Project:
    ...
    def __init__(self, root):
        ...
        self.variants_path = os.path.join(self.root, "js", "variants.js")
        ...
        self.monsters = load_monsters(self.root)
        apply_monster_prompts(self)
    ...
    # ── per-project helpers ────────────────────────────────────────────
    # ── monster variants manifest (js/variants.js) ─────────────────────
    # gen_monsters.py owns the format; the helper reads and rewrites the same
    # file so a pick made in the debug panel is a committable change, not just
    # a localStorage entry on one device.
    def load_variants(self):
        try:
            with open(self.variants_path, "r", encoding="utf-8") as handle:
                raw = handle.read()
            data = json.loads(raw[raw.index("{"):raw.rindex("}") + 1])
        except (FileNotFoundError, ValueError):
            data = {}
        data.setdefault("game", self.slug)
        data.setdefault("monsters", [])
        data.setdefault("clips", {})
        data.setdefault("zoom", {})
        return data

    def save_variants(self, data):
        data["generated"] = time.strftime("%Y-%m-%d %H:%M:%S")
        os.makedirs(os.path.dirname(self.variants_path), exist_ok=True)
        with open(self.variants_path, "w", encoding="utf-8") as handle:
    ...
    def list_markers(self):
        ...
        for file_name in sorted(os.listdir(self.ref_dir)):
            ...
            if not lower.endswith((".jpg", ".jpeg", ".png")):
                continue
            if lower.startswith("monster_"):
                continue  # identity references, not marker frames
            path = os.path.join(self.ref_dir, file_name)
            stat = os.stat(path)
            markers.append({
                "file": file_name,
    ...
    def list_transitions(self):
        ...
        for file_name, transition in self.event_transitions.items():
            ...
            rows.append({
                ...
                "file": file_name,
                "src": self.video_row_src(file_name),
                "poster": transition.get("poster") if transition.get("promptFromConfig") else (meta.get("poster") or transition.get("poster", transition["start"])),
                "startImage": transition["start"],
                "endImage": transition.get("end", ""),
                "promptText": transition["promptText"] if transition.get("promptFromConfig")
                              else (meta.get("promptText") or transition["promptText"]),
                "status": status,
                "canRedo": True,
                "canReverse": False,
                "canMarker": True,
...
def next_variant_file(project, kind, monster_id):
    """Monster redos never overwrite: each render lands as the next numbered
    variant so the panel can compare every attempt and the reviewer picks."""
    data = project.load_variants()
    entry = data["clips"].setdefault(f"{kind}:{monster_id}", {"selected": "", "variants": []})
    number = max([variant.get("n", 0) for variant in entry["variants"]] + [0]) + 1
    return data, entry, number, f"monster_{kind}_{monster_id}_v{number}.mp4"
...
def process_regen(project, local_job):
    ...
    monster_id, monster_kind = monster_id_from_transition(file_name)
    if monster_id:
        return process_monster_regen(project, local_job, transition, monster_id, monster_kind, prompt_text)
...
def process_monster_regen(project, local_job, transition, monster_id, kind, prompt_text):
    """Render one more take of a monster beat and append it to the manifest.
    ...
    local_job["bytes"] = os.path.getsize(out_path)
    local_job["duration_secs"] = ltx_job.get("duration_secs")
    # Re-read: a concurrent pick may have rewritten the file while LTX rendered.
    data = project.load_variants()
    entry = data["clips"].setdefault(f"{kind}:{monster_id}", {"selected": "", "variants": []})
    entry["variants"].append({
        ...
        "bytes": local_job["bytes"],
        "prompt": prompt_text,
        "start": render_options.get("start_image", ""),
        "note": "debug redo",
        "durationSecs": ltx_job.get("duration_secs"),
    })
    if not entry.get("selected"):
        entry["selected"] = f"videos/{out_file}"
    # Attack takes get the jump-scare punch armed by default (see
    # DEFAULT_ATTACK_ZOOM in gen_monsters.py) — a reviewer only has to move the
    # tap point, not discover the feature.
    if kind == "attack" and f"videos/{out_file}" not in data["zoom"]:
        data["zoom"][f"videos/{out_file}"] = {
            "enabled": True, "x": 0.5, "y": 0.4, "scale": 2.6, "lead": 1.0, "fade": 0.4,
        }
    project.save_variants(data)
    local_job["variant"] = out_file
    local_job["status"] = "done"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if slug not in PROJECTS:
            return self.send_json({"ok": False, "error": f"unknown project: {slug}"}, 404)
        if rest not in {"/api/regen", "/api/reverse", "/api/marker", "/api/image_preview",
                        "/api/image_commit", "/api/variant_select", "/api/variant_zoom"}:
            return self.send_json({"ok": False, "error": "not found"}, 404)
        project = PROJECTS[slug]
        ...
        # Variant picking and zoom tuning are plain manifest edits — no queue,
        # no render — so the panel stays responsive while a job is in flight.
        if rest == "/api/variant_select":
            kind = data.get("kind")
            monster = str(data.get("monster", ""))
            src = str(data.get("src", ""))
            if kind not in {"release", "attack"} or not monster or not src:
                return self.send_json({"ok": False, "error": "kind, monster and src are required"}, 400)
            manifest = project.load_variants()
            entry = manifest["clips"].get(f"{kind}:{monster}")
            if not entry or not any(item.get("src") == src for item in entry["variants"]):
                return self.send_json({"ok": False, "error": "unknown variant"}, 400)
            entry["selected"] = src
            project.save_variants(manifest)
            return self.send_json({"ok": True, "selected": src})
        if rest == "/api/variant_zoom":
            src = str(data.get("src", ""))
            zoom = data.get("zoom")
            if not src or not isinstance(zoom, dict):
                return self.send_json({"ok": False, "error": "src and zoom are required"}, 400)
            manifest = project.load_variants()
            manifest["zoom"][src] = {
                "enabled": bool(zoom.get("enabled")),
                "x": float(zoom.get("x", 0.5)),
                "y": float(zoom.get("y", 0.42)),
                "scale": float(zoom.get("scale", 2.4)),
                "lead": float(zoom.get("lead", 0.9)),
                "fade": float(zoom.get("fade", 0.45)),
            }
            project.save_variants(manifest)
            return self.send_json({"ok": True, "zoom": manifest["zoom"][src]})
        if rest == "/api/marker":
