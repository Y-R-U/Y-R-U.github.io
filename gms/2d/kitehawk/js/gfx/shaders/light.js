...
layout(location=1) in vec4 i_posRad;   // x, y, radius, parallaxX
layout(location=2) in vec4 i_color;    // rgb * intensity, softness
layout(location=3) in vec4 i_shape;    // squash (y scale), angle, parallaxY, 0
...
void main() {
  ...
  vec2 px = (world - u_cam * vec2(i_posRad.w, i_shape.z)) * u_scale;
