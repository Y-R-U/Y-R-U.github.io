...
import { fbm1, clamp01 } from './gmath.js';
...
export function createPostFX(gl, quadBuf, opts = {}) {
  ...
  let gLoadAmt = 0, gLoadBaseVig = 0.55, gLoadBaseSat = 1.06;
  ...
  const fx = {
    ...
    /**
     * G-load veiling. Positive = greyout (vignette closes, colour drains),
     * negative = redout. One call so no scene has to remember the three knobs
     * that make it read; no shader change.
     */
    gLoad(amount) {
      const a = amount > 1 ? 1 : amount < -1 ? -1 : amount;
      gLoadAmt = a;
      const m = Math.abs(a);
      fx.vignetteAmt = gLoadBaseVig + (1 - gLoadBaseVig) * (m * m);
      fx.saturation = gLoadBaseSat * (1 - 0.92 * m);
      if (a < 0) fx.flash(1.0, 0.10, 0.06, 0.42 * m * m, 0.05);
    },
    get gLoadAmt() { return gLoadAmt; },
    /** Call once per scene, after the base vignette/saturation are set. */
    gLoadRebase() { gLoadBaseVig = fx.vignetteAmt; gLoadBaseSat = fx.saturation; },
    ...
    reset() {
      ...
      gLoadAmt = 0; fx.vignetteAmt = gLoadBaseVig; fx.saturation = gLoadBaseSat;
