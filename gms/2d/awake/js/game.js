(function () {
  ...
  // whether you're on http://127.0.0.1:8788/awake/?debug (helper-served,
  // same origin) or your own static server at
  // http://localhost:8778/gms/2d/awake/ (cross-origin, CORS handles it).
  // The helper hosts every project under its own slug, so we extract the
  // slug from the URL path.
  const helperUrl = (function () {
    const m = location.pathname.match(/\/gms\/2d\/([^\/]+)\//)
           || location.pathname.replace(/^\//, "").match(/^([^\/]+)/);
    const slug = (m && m[1]) || "awake";
  ...
  function init() {
    ...
    syncSettings();
    renderDebugList();
    // Helper poll runs only while the debug panel is open. On the deployed
    // HTTPS site every call to the local http helper is blocked as mixed
    // content, and a 5s background poll turns that into a console full of
    // errors for players who never open the panel.
    showIntro();
  ...
  function wireEvents() {
    ...
    // The close button goes through UI's [data-close] handler, so watch the
    // panel's class instead of trying to wedge a hook in there.
    const debugPanel = document.getElementById("debug-panel");
    if (debugPanel) {
      const observer = new MutationObserver(() => {
        if (debugPanel.classList.contains("open")) startHelperPoll();
        else stopHelperPoll();
      });
  ...
  async function playMonsterRelease(message) {
    ...
    // Hold the message ~3× the old timing before offering "continue".
    await delay(2000);
  ...
  function renderEnding(kind) {
    ...
    if (eventClip && Monsters && !success) Monsters.punch(els.endingVideo, eventClip);
  ...
  function renderStats() {
    ...
    // and side-panel copy (desktop only).
