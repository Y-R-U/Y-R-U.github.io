...
def main():
    ...
    for item in load_events():
        ...
        if wanted and stem not in wanted and item["output"] not in wanted and item["group"] not in wanted:
            continue
        # Monster clips moved to gen_monsters.py, which renders numbered
        # variants instead of overwriting. The file this script would replace is
        # variant v1 in js/variants.js, so clobbering it would rewrite history.
        if item["group"] in {"monster_release", "monster_attack"}:
            log(f"skip {item['output']} — monster clips are generated by gen_monsters.py")
            continue
