...
export function configure(quality) {
  // main.js configures before ?preset= is applied, so without this the cap is read once at the
  // default and every preset bakes identical textures. materials/index.js already subscribes.
  if (!subscribed) { subscribed = true; quality.onChange(() => configure(quality)); }
