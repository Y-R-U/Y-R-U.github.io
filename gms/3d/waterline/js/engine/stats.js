...
// The mid-phone gate, BUILD_PLAN §6. Tighter than FORGE's on counts and texture memory: no
// terrain, no foliage atlas. The bridge shots are tighter still (120 calls / 260k tris) — those
// are checked in review, the HUD grades against the looser sea/cine numbers.
export const BUDGETS = { gpu: 11, cpu: 6, calls: 120, tris: 300e3, texMB: 45, hitch: 50 };
...
export class Stats {
  ...
  verdict() {
    ...
    // 1.25, not 1.06: one 18–19 ms frame in 90 is ordinary vsync jitter, and at 1.06 a scene
    // drawing 1.5 ms of GPU work reported "tight".
    if (miss > 1.6) label = 'struggling';
    else if (miss > 1.25 && label !== 'struggling') label = 'tight';
  ...
  render() {
    if (!this.el) { this.snapshot = this.read(); return; }
    ...
    this.el.innerHTML = `
      ...
      <div class="perf-grid">
        ...
        ${cell(s.hitchMs.toFixed(0), 'ms worst frame', s.hitchMs, BUDGETS.hitch)}
      </div>`;
  }
}
