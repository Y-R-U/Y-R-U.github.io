// One rigged-humanoid bone driver, shared by every enemy. The models are
// PolyPerfect SkinnedMeshes that all share one 80-bone family. Each controlled
...
//
// (Ported from deadtown js/hero.js — the proven driver.)
...
export function buildRig(srcScene, { tint = null, scale = 0.96, emissive = 0x550000 } = {}) {
  ...
  // attach points for crowns / carried props
  const headAttach = new THREE.Object3D();
  B('Head_M')?.add(headAttach);

  return { group, model, parts, B, headAttach, DOWN_R, DOWN_L };
}
