...
import {VISUAL_WIND} from '../core/visual-config.mjs';
...
export function createBoatView(){
  ...
  boat.userData={triangles,draws:4,sections:7,discover(time){discoveryTime=time;},update(time,speed,yaw=0){materials[3].emissiveIntensity=1.1+3*Math.max(0,1-(time-discoveryTime)/4);const p=boat.children[0].geometry.attributes.position;const wx=VISUAL_WIND.x*Math.cos(yaw)-VISUAL_WIND.z*Math.sin(yaw),wz=VISUAL_WIND.x*Math.sin(yaw)+VISUAL_WIND.z*Math.cos(yaw)-speed/VISUAL_WIND.speed;
    const length=Math.hypot(wx,wz)||1;p.array[flagStart+3]=.42*wx/length;p.array[flagStart+5]=2.16+.42*wz/length+Math.sin(time*5)*.025;p.needsUpdate=true;}};
  return boat;
}
