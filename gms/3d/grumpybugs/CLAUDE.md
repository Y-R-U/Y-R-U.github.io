# GRUMPY BUGS — 3D worms-with-insects artillery
...
Forked from **grudgebugs** (`../grudgebugs/`) on 2026-07-28 as an art
experiment — same gameplay, rebuilt visuals. The save key is
`grumpybugs_v1`, so the two builds keep separate profiles.

## The look (2026-07-28 art pass)

`js/render.js` owns tone mapping, shadow quality, the light rig and the
environment probe, and **everything** renders through it — the game, the intro
cutscene and `tools/gallery.html`. ACES tone mapping + a warm key / cool sky
bounce / hard back rim is what stops flat-shaded primitive soup reading as
plastic; the PMREM probe (a 8×64 gradient canvas) is what makes `metal > 0`
mean anything.

**`tools/gallery.html` is the art workflow.** It lays every prop out on a
turntable under the real light rig, each auto-fitted to the same cell, so a
berry and THE SHOE can be judged side by side:
`tools/gallery.html?set=weapons|bugs|hats|fx&only=<label>&spin=0&y=<yaw>`.
...
themes, AI diffs, camera params) · `js/render.js` **tone mapping / light rig /
env probe** · `js/utils.js` PRNG/vec helpers ·
...
- **Projectiles fly +Z, and `lookAt` wipes the euler every frame.** game.js
  `lookAt`s each projectile down its path, so assigning `mesh.rotation.z += …`
  for spin does nothing — the next lookAt overwrites it. Roll is a *relative*
  `rotateZ`/`rotateX` applied after, inside `animateProjectile(mesh, dt, roll)`,
  which also drives fuse sparks, the rocket plume, the dung flies and the
  loogie wobble off `userData.anim`. Same deal for `animateReticle`.
- **Bug materials are `metal: 0.2`+ chitin**, which only looks right with
