// GRUMPY BUGS — the battle engine. Turn state machine, aiming and firing,
...
import {
  makeProjectile, makeShoe, makeBomber, animateBomber, makeBeeBomb,
  makeReticle, makeTrajectory, setTrajectory, animateProjectile, animateReticle,
} from './weapons.js';
...
export class Battle {
  ...
  constructor(scene, deps, opts) {
    this.scene = scene;
    this.fx = deps.fx; this.cams = deps.cams; this.dom = deps.dom; this.renderer = deps.renderer;
    this.opts = opts;
    ...
    this.arena = new ArenaView(scene, opts.theme, this.ledges, this.rng,
      { lite: opts.lite, sandwich: opts.sandwich, sandwichPos: opts.sandwichPos, renderer: deps.renderer });
  ...
  _replayStep(dt) {
    ...
    for (const g of rp.ghosts) {
      if (g.kind === 'proj') {
        ...
        animateProjectile(g.mesh, dt, dt * 6);
      } else {
  ...
  update(dt, unscaledDt) {
    ...
    if (this.reticle.visible) animateReticle(this.reticle, unscaledDt);
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      // lookAt rewrites the whole euler every frame, so the roll has to be a
