...
import { TANK, ARENA_R } from './config.js';
...
import { spawnExplosion, spawnFlash, spawnDebris } from './particles.js';
...
export class Tank {
  ...
  updatePhysics(dt) {
    ...
    // hover bob + lean
    this.leanG.position.y = 0.32 + Math.sin(state.time * 3.1 + this.bobPhase) * 0.06;
    const cos = Math.cos(this.yaw), sin = Math.sin(this.yaw);
    const localVx = this.vel.x * cos - this.vel.z * sin;
    const localVz = this.vel.x * sin + this.vel.z * cos;
    this.leanG.rotation.x = lerp(this.leanG.rotation.x, clamp(-localVz * 0.016, -0.18, 0.18), damp(8, dt));
    this.leanG.rotation.z = lerp(this.leanG.rotation.z, clamp(localVx * 0.02, -0.2, 0.2), damp(8, dt));

    for (let i = 0; i < this.padMats.length; i++) {
      this.padMats[i].opacity = 0.65 + Math.sin(state.time * 9 + i * 1.7 + this.bobPhase) * 0.3;
    }
