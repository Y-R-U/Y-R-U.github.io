...
export class Interior {
  constructor(zoneId, house, opts = {}) {
    ...
    const twoUp = wallTop - plinth > 6.6;
...
function shell(b, I, z, R) {
  ...
  for (const [fi, f] of face.entries()) {
    ...
    if (fi === 1) {
      ...
      for (const s of [-1, 1]) {
        b.add('wood', box(side, wallH, 0.075), f.m.clone().multiply(T(s * (dw + side) / 2, fy + wallH / 2, 0.038)));
        b.add('wood', box(side * 0.94, panelH - fy, 0.09), f.m.clone().multiply(T(s * (dw + side) / 2, (fy + panelH) / 2, 0.113)));
      }
      b.add('wood', box(dw, wallH - dh, 0.075), f.m.clone().multiply(T(0, fy + dh + (wallH - dh) / 2, 0.038)));
      ...
      b.add('wood', box(wide, wallH, 0.075), f.m.clone().multiply(T(0, fy + wallH / 2, 0.038)));
      // the window wall is left plain, or the dado cuts the leaded light in half
      if (fi !== 0) b.add('wood', box(wide * 0.99, panelH - fy, 0.09), f.m.clone().multiply(T(0, (fy + panelH) / 2, 0.113)));
    }
    if (fi !== 0) b.add('wood', box(wide, 0.135, 0.225), f.m.clone().multiply(T(0, panelH, 0.128)));
...
function hearth(b, I) {
  ...
  const bw = Math.min(rz * 1.1, 2.85), bh = Math.min(roomH * 0.82, 3.45);
  const x = -rx + 0.09;
