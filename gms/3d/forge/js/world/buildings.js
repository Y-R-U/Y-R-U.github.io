...
export const TUNING = {
  wallSeg: 4.6,       // metres per curtain-wall module
  buttressEvery: 3,   // segments between buttresses
  panelT: 0.34,       // house wall thickness — this is what makes a reveal read
  eaves: 0.55,        // roof overhang
...
export function wallRun(zoneId, { length = 40, height = 8, thickness = 2.4, seed: sv = 0 } = {}) {
  ...
  b.add(S.wall, taperBox(length, thickness, foot, length, thickness + 1.15), T(0, foot / 2, 0));
  addSkirt(b, { m: T(0, 0, 0), length: length + 0.5, thickness: thickness + 1.15, h: 0.34 });
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (inGate(i)) {
      ...
      for (const s of [-1, 1]) openings.push({ kind: z.window.shape, w: 0.75, h: 1.6, x: s * (gateW * 0.32), y: gateH + 1.8, reveal: thickness * 0.3 });
      openings.push({ kind: 'square', w: 0.32, h: 1.5, x: 0, y: gateH + 1.9, glass: false, bars: false, sill: false, head: false, reveal: thickness * 0.5 });
    } else if (R() < 0.42) {
      openings.push({ kind: 'square', w: 0.3, h: 1.5, x: span(R, -0.6, 0.6), y: top * 0.5, glass: false, bars: false, sill: false, head: false, reveal: thickness * 0.62 });
    }

    // the gate module is a thicker block, so the gatehouse projects from both wall faces
    const pt = inGate(i) ? thickness + 1.3 : thickness;
  ...
  for (let i = 1; i < n; i++) {
    ...
    for (const side of [-1, 1]) {
      ...
      const sd = side * (thickness / 2 + 0.5);
      b.add(S.wall, taperBox(1.4, 1.1, bh, 1.7, 1.75), T(x, foot + bh / 2, sd));
      b.add(S.trim, taperBox(1.5, 1.25, 0.4, 1.62, 1.9), T(x, foot + bh + 0.2, sd));
      b.add(S.wall, taperBox(0.9, 0.28, 0.55, 1.35, 1.15), T(x, foot + bh + 0.66, sd - side * 0.34));
    }
  }
  ...
  for (let i = 1; i <= n; i++) {
    ...
    if (i0 === ruin) {
      ...
      for (let k = 0; k < 6; k++) {
        const bw = len / 6;
        const bh = span(R, 0.12, 0.95) * (1 - Math.abs(k - 2.5) / 4);
        b.add(S.wall, new THREE.BoxGeometry(bw * 0.94, bh, thickness * span(R, 0.7, 1.0)),
          T(x0 + bw * (k + 0.5), ht + bh / 2, span(R, -0.3, 0.3)));
      }
      for (const side of [-1, 1]) {
        addRubble(b, R, { m: T(mid, 0, side * (thickness / 2 + 1.5)), length: len * 1.2, offset: side * 1.1, count: 16, size: 0.62, surface: S.trim });
      }
    } else if (ht !== houseH) {
      b.add(S.trim, taperBox(len, thickness + 0.28, 0.26, len, thickness + 0.1), T(mid, foot + (ht - foot) * 0.5, 0));
      addCorbels(b, { m: T(mid, ht, 0), length: len, thickness, surface: S.trim });
      b.add(S.trim, new THREE.BoxGeometry(len, 0.34, thickness + 0.95), T(mid, ht + 0.17, 0));
      addMerlons(b, R, { m: T(mid, ht + 0.34, 0), length: len, thickness: thickness * 0.68, style: z.edges, height: 1.3, step: 1.95, surface: S.wall, cap: S.trim, crest: z.crest, crestSurface: S.crest });
    }
    i0 = i;
  }

  if (hoard >= 0) {
    const hx = -length / 2 + segL * (hoard + 0.5), hy = segTop[hoard] + 0.5;
    const hl = segL * 1.55, out = thickness / 2 + 1.5;
    for (const side of [-1, 1]) {
      const zc = side * out;
      b.add(S.trim, new THREE.BoxGeometry(hl, 0.2, 1.9), T(hx, hy, zc));
      b.add(S.trim, new THREE.BoxGeometry(hl, 1.5, 0.14), T(hx, hy + 0.75, zc + side * 0.9));
      b.add(S.trim, new THREE.BoxGeometry(hl + 0.5, 0.16, 2.6),
        T(hx, hy + 2.05, zc + side * 0.2, 0, side * 0.34));
      for (let k = 0; k < 3; k++) {
        const px = hx + (k - 1) * hl * 0.42;
        b.add(S.trim, new THREE.BoxGeometry(0.2, 2.0, 0.2), T(px, hy + 1.05, zc + side * 0.85));
        b.add(S.trim, new THREE.BoxGeometry(0.18, 2.3, 0.18), T(px, hy - 0.95, zc - side * 0.35, 0, side * 0.5));
      }
    }
  }
  ...
  if (gate) {
    ...
    for (const side of [-1, 1]) {
      const px = gx + side * (gateW / 2 + 0.95);
      b.add(S.wall, taperBox(1.2, thickness + 1.1, houseH - 1.1, 1.5, thickness + 1.7), T(px, (houseH - 1.1) / 2, 0));
    }
    const bar = 0.075;
    ...
    const gw = gateSpan * segL + 0.5, gd = thickness + 1.5;
    addCorbels(b, { m: T(gx, houseH, 0), length: gw, thickness: gd - 0.5, surface: S.trim });
    b.add(S.trim, new THREE.BoxGeometry(gw, 0.4, gd + 0.3), T(gx, houseH + 0.2, 0));
    const grise = gd * (z.edges === 'sharp' ? 0.95 : 0.66);
    const gRoof = roofSlab({ w: gd, d: gw, rise: grise, over: 0.4, th: z.roof.tile === 'thatch' ? 0.5 : 0.32, profile: z.edges === 'curved' ? 'curved' : 'flat' });
    gRoof.rotateY(Math.PI / 2);
    b.add(S.roof, gRoof, T(gx, houseH + 0.4, 0), true);
    b.add(S.trim, new THREE.BoxGeometry(gw + 0.9, 0.2, 0.42), T(gx, houseH + 0.4 + grise - 0.07, 0));
    addCrest(b, z.crest, R, { m: T(gx, houseH + 0.44 + grise, 0), length: gw * 0.8, surface: S.crest });
    for (const side of [-1, 1]) {
      panel(b, S, {
        m: T(gx + side * (gw / 2 - 0.3), houseH + 0.4, 0, side * Math.PI / 2), t: 0.3,
        shape: gableShape(gd, grise, 0.4, z.edges === 'curved' ? 'curved' : 'flat', 0.32),
        openings: [{ kind: z.window.shape, w: 0.6, h: 1.1, x: 0, y: 0.25, reveal: 0.24, sill: false }],
      });
    }
  }
...
export function tower(zoneId, { radius = 4, height = 18, sides = 16, seed: sv = 0 } = {}) {
  ...
  b.add(S.wall, new THREE.CylinderGeometry(radius * 1.34, radius * 1.5, 0.4, n), T(0, 0.2, 0));

  const core = Math.max(0.6, apo - t - 0.06);
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let L = 0; L < levels.length; L++) {
      ...
      const big = L > 0 && lit > 0;
      openings.push(big
        ? { kind: z.window.shape, w: Math.min(pw * 0.5, 1.05), h: 1.9, x: 0, y: levels[L], reveal: t * 0.8 }
        : { kind: 'square', w: 0.28, h: 1.4, x: 0, y: levels[L], glass: false, bars: false, sill: false, head: false, reveal: t * 0.72 });
    }
    panel(b, S, { m, w: pw + 0.03, h: shaftH, t, openings });
  }

  for (const f of [0.36, 0.7]) {
    b.add(S.trim, new THREE.CylinderGeometry(radius + 0.14, radius + 0.14, 0.24, n), T(0, foot + shaftH * f, 0));
  }
  ...
  for (let i = 0; i < n * 2; i++) {
    ...
    b.add(S.trim, taperBox(0.3, 0.5, 0.5, 0.16, 0.26), T(Math.sin(a) * (radius + 0.16), topY - 0.25, Math.cos(a) * (radius + 0.16), a));
  }
  const ring = radius * 1.26;
  b.add(S.trim, new THREE.CylinderGeometry(ring, radius + 0.1, 0.46, n), T(0, topY + 0.23, 0));

  const mw = 2 * ring * Math.sin(Math.PI / n) * 0.66;
  for (let i = 0; i < n; i++) {
    if (R() < 0.08) continue;
    const a = (i / n) * Math.PI * 2;
    const mh = 1.35 * span(R, 0.9, 1.12);
    const m = T(Math.sin(a) * (ring - 0.22), topY + 0.46, Math.cos(a) * (ring - 0.22), a);
    b.add(S.wall, new THREE.BoxGeometry(mw, mh, 0.5), m.clone().multiply(T(0, mh / 2, 0)));
    if (z.edges === 'sharp') b.add(S.trim, new THREE.ConeGeometry(mw * 0.7, 0.34, 4).rotateY(Math.PI / 4), m.clone().multiply(T(0, mh + 0.17, 0)));
    else if (z.edges === 'curved') b.add(S.trim, new THREE.CylinderGeometry(mw * 0.46, mw * 0.46, 0.58, 7, 1, false, 0, Math.PI).rotateX(-Math.PI / 2), m.clone().multiply(T(0, mh - 0.02, 0)));
    else b.add(S.trim, new THREE.BoxGeometry(mw + 0.14, 0.12, 0.6), m.clone().multiply(T(0, mh + 0.06, 0)));
    if (z.crest.type === 'spikes' && R() < z.crest.density) {
      const sh = span(R, 0.7, 1.1);
      b.add(S.crest, new THREE.ConeGeometry(0.075, sh, 4).rotateY(Math.PI / 4), m.clone().multiply(T(0, mh + 0.34 + sh / 2, 0)));
    }
  }
  ...
  const prof = [V2(rr * 1.12, -0.1), V2(rr * 1.14, 0.16)];
  for (let i = 1; i <= 5; i++) {
    const u = i / 5;
    prof.push(V2(rr * (1 - u), 0.3 + rh * Math.pow(u, curve)));
  }
  const roofY0 = topY + 1.6;
  b.add(S.roof, new THREE.LatheGeometry(prof, n), T(0, roofY0, 0));
  b.add(S.crest, new THREE.SphereGeometry(0.24, 7, 5), T(0, roofY0 + 0.3 + rh + 0.1, 0));

  if (R() < 0.7) {
    const ph = 2.4;
    b.add(S.trim, new THREE.CylinderGeometry(0.06, 0.07, ph, 5), T(0, roofY0 + 0.4 + rh + ph / 2, 0));
    const fl = new THREE.BufferGeometry();
    const fy = roofY0 + 0.4 + rh + ph - 0.25;
    fl.setAttribute('position', new THREE.Float32BufferAttribute([0, 0, 0, 1.5, -0.28, 0, 0, -0.95, 0, 0, 0, 0, 0, -0.95, 0, 1.5, -0.28, 0], 3));
...
export function house(zoneId, { w = 8, d = 7, h = 6, seed: sv = 0 } = {}) {
  ...
  const plinth = 0.44;
  const wallTop = h;
  const storeys = wallTop - plinth > 4.4 ? 2 : 1;
  ...
  const winW = kind === 'lancet' ? 0.74 : 0.95;
  ...
  for (const [fi, f] of faces.entries()) {
    ...
    for (const [ri, ry] of rows.entries()) {
      for (let i = 1; i <= slots; i++) {
        ...
        if (fi === doorFace && ri === 0 && slots > 1 && i === Math.ceil(slots / 2)) continue;
        if (R() < 0.1) continue;
  ...
  if (storeys === 2) b.add(S.trim, new THREE.BoxGeometry(w + 0.24, 0.22, d + 0.24), T(0, plinth + (wallTop - plinth) * 0.5 - 0.35, 0));

  // half-timber relief on the upper band of the two long faces
  if (dressed && storeys === 2 && R() < 0.6) {
    const y0 = plinth + (wallTop - plinth) * 0.5 - 0.24, bandH = wallTop - y0 - 0.1;
    for (const f of [faces[0], faces[1]]) {
      const cols = Math.max(2, Math.round(f.span / 1.6));
      for (let i = 0; i <= cols; i++) {
        b.add(S.trim, new THREE.BoxGeometry(0.15, bandH, 0.07), f.m.clone().multiply(T(-f.span / 2 + f.span * i / cols, y0 + bandH / 2, t + 0.035)));
      }
      b.add(S.trim, new THREE.BoxGeometry(f.span, 0.16, 0.07), f.m.clone().multiply(T(0, y0 + bandH, t + 0.035)));
    }
  }
  ...
  for (const gm of gEnds) {
    const openings = [];
    if (R() < 0.85) {
      const pair = spanW > 7.5 && rise > 3.2 && R() < 0.5;
      const gh = Math.min(1.5, rise * 0.42);
      for (const gx of pair ? [-0.85, 0.85] : [0]) openings.push({ kind, w: winW * 0.85, h: gh, x: gx, y: rise * 0.16, reveal: t * 0.75 });
    }
    panel(b, S, { m: gm, t, shape: gShape(), openings });
  }

  const ridgeM = ridgeX ? T(0, wallTop + rise, 0) : T(0, wallTop + rise, 0, Math.PI / 2);
  b.add(S.trim, new THREE.BoxGeometry(ridgeLen + over * 1.6, 0.2, 0.44), ridgeM.clone().multiply(T(0, -0.07, 0)));
  ...
  for (const end of [-1, 1]) for (const s of [-1, 1]) {
    const m = ridgeM.clone().multiply(T(end * (ridgeLen / 2 + over + 0.05), 0, 0, 0, 0, 0));
    b.add(S.trim, new THREE.BoxGeometry(0.12, 0.24, bl), m.clone().multiply(T(0, -rise / 2 - 0.06, s * (spanW / 4 + over / 2), 0, s * slopeA)));
  }
  ...
  if (R() < 0.5 && spanW > 5.5) {
    const dw = 1.75, dr = 0.9, dOver = 0.16, dTh = th * 0.7;
    const off = spanW * 0.3;
    ...
    const fm = ridgeX
      ? T(along, yOnRoof - 0.5, sideSign * off, sideSign > 0 ? 0 : Math.PI)
      : T(sideSign * off, yOnRoof - 0.5, along, sideSign > 0 ? Math.PI / 2 : -Math.PI / 2);
    panel(b, S, { m: fm.clone().multiply(T(0, 0, 0.5)), w: dw, h: 1.5, t: 0.24, openings: [{ kind, w: 0.62, h: 1.0, x: 0, y: 0.3, reveal: 0.2, sill: false }] });
    panel(b, S, { m: fm.clone().multiply(T(0, 1.5, 0.5)), t: 0.24, shape: gableShape(dw + 0.3, dr, dOver, profile, dTh) });
    b.add(S.roof, roofSlab({ w: dw + 0.3, d: 1.5, rise: dr, over: dOver, th: dTh, profile }), fm.clone().multiply(T(0, 1.5, 0)), true);
  }

  // lean-to: breaks the eaves line and gives the roofline a second, lower step
  if (R() < 0.45) {
    const lw = span(R, 1.7, 2.5), ld = Math.min(d * 0.7, 4.6), lh = wallTop * span(R, 0.42, 0.56);
    const sx = R() < 0.5 ? 1 : -1;
    const lz = span(R, -d * 0.12, d * 0.12);
    const pitch = Math.atan2(lw * 0.62, lw + 0.6);
    b.add(S.wall, taperBox(lw, ld, lh, lw + 0.22, ld + 0.22), T(sx * (w / 2 + lw / 2 - 0.12), lh / 2, lz));
    b.add(S.roof, new THREE.BoxGeometry((lw + 0.6) / Math.cos(pitch), th * 0.8, ld + 0.5),
      T(sx * (w / 2 + lw / 2 - 0.25), lh + lw * 0.32, lz, 0, 0, -sx * pitch));
  }

  // projecting gabled bay: the cheapest way to stop a house reading as one box
  if (R() < 0.5) {
    const f = faces[1 + Math.floor(R() * 3)];
    const bw = Math.min(f.span * 0.36, 2.6), bd = 1.25, bh = wallTop * span(R, 0.44, 0.62);
    const om = f.m.clone().multiply(T(span(R, -f.span * 0.2, f.span * 0.2), 0, t));
    const brise = (bw / 2 + 0.32) * Math.max(0.85, pitchBase * 1.35);
    b.add(S.wall, taperBox(bw + 0.28, bd + 0.5, plinth, bw + 0.85, bd + 1.0), om.clone().multiply(T(0, plinth / 2, bd / 2 - 0.3)));
    b.add(S.wall, new THREE.BoxGeometry(bw, bh - plinth, bd + 0.4), om.clone().multiply(T(0, plinth + (bh - plinth) / 2, bd / 2 - 0.35)));
    ...
    b.add(S.roof, roofSlab({ w: bw, d: bd + 0.5, rise: brise, over: 0.32, th: th * 0.85, profile }), om.clone().multiply(T(0, bh, bd / 2 - 0.35)), true);
    panel(b, S, { m: om.clone().multiply(T(0, bh, bd - t)), t, shape: gableShape(bw, brise, 0.32, profile, th * 0.85) });
  }
