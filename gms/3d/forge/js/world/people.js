...
export class People {
  ...
  spawn() {
    ...
    for (let n = 0; agents.length < POOL && n < POOL * 40; n++) {
      ...
      if (kind === 'road') {
        // Bounded to the town, not to the creek: A4 moved the creek 100 m further south and a
        // walker pool spread over 200 m of road empties the street the shots actually look down.
        const zEnd = Math.min(cz + 12, 62);
        const z0 = span(R, -30, zEnd);
        const off = span(R, -2.3, 2.3);
        const x = roadX(pts, z0) + off;
        if (!free(x, z0)) continue;
        a = { kind: 'walk', pts, off, z: z0, zMin: -32, zMax: zEnd + 5, dir: R() < 0.5 ? 1 : -1,
