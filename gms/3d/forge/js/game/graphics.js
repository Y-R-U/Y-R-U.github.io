...
export const SHADOW_MODES = ['off', 'hard', 'soft'];
...
export const SHADOW_ROWS = [['off', 'Off'], ['hard', 'Hard'], ['soft', 'Soft']];

// Ultra used to ask for three's PCFSoft variant. The renderer still understands it and the
// developer panel can still reach it, but no preset and no save may carry it: left alone it draws
// the shadow row with nothing selected at all.
export const shadowMode = v => (v === 'softhigh' ? 'soft' : SHADOW_MODES.includes(v) ? v : null);
...
export function resolve(settings = {}, fallback = DEFAULT_PRESET) {
  ...
  const s = { ...settings, shadows: shadowMode(settings.shadows) };
