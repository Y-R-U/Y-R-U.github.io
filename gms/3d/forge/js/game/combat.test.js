...
// Enough browser to construct a real Session: it builds the HUD, the menu and the market out of
// DOM nodes, and `start()` fetches the packs off disk. Everything it does with them is append and
// classList, so a bag with those on it is all a node run needs — and a Session that is really
// constructed is the only way to test what its constructor decides.
function fakeDom() {
  const node = () => {
    const n = {
