// Rendering setup + the low-poly harvest-dusk farm: gradient sky, setting
// sun, faceted hill terrain, wheat stubble, fence ring, dead trees, a barn
// with a lit window, a windmill, hay bales, scarecrows and fireflies.
...
function buildSky() {
  const skyMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // warm bias toward the sun azimuth (-z, slightly +x)
  ...
  // pale rising moon, opposite side
  ...
  // stars in the darker half of the sky
  const starCount = 380;
  ...
  // slow flat clouds catching the last light
  const cloudMat = new THREE.MeshStandardMaterial({
    color: 0x352040, emissive: 0x451a2e, emissiveIntensity: 0.5,
    flatShading: true, roughness: 1, fog: false });
  for (let i = 0; i < 7; i++) {
    // long thin stratus bands, not puffs — dusk clouds read as dark streaks
...
// ---------------------------------------------------------------------------
// Terrain: one big plane, flat across the field, faceted hills beyond
// ---------------------------------------------------------------------------
...
function buildTerrain() {
  ...
  for (let i = 0; i < pos.count; i++) {
    ...
    // rolling hills fade in past the fence; big swells far out
    ...
    // gentle per-vertex variation keeps the flat field from looking sterile
...
// ---------------------------------------------------------------------------
// The field: stubble, fence ring, hay bales, scarecrows, pumpkins
// ---------------------------------------------------------------------------

function buildField() {
  // wheat stubble — one instanced mesh
  ...
  // fence: instanced posts + two rail rings
  ...
  // hay bales (collidable)
  const hayMat = new THREE.MeshStandardMaterial({
    color: 0xa87f33, flatShading: true, roughness: 1 });
  const hayGeo = new THREE.CylinderGeometry(1.25, 1.25, 2.3, 9);
  for (let i = 0; i < 6; i++) {
    const a = rand(0, Math.PI * 2);
    const r = rand(14, FIELD_R - 8);
    const bale = new THREE.Mesh(hayGeo, hayMat);
    bale.rotation.z = Math.PI / 2;
    bale.rotation.y = rand(0, Math.PI);
    bale.position.set(Math.cos(a) * r, 1.25, Math.sin(a) * r);
    bale.castShadow = true;
    bale.receiveShadow = true;
    scene.add(bale);
    obstacles.push({ x: bale.position.x, z: bale.position.z, r: 1.9 });
  }

  // scarecrows (collidable) — the crows are not scared
  for (const a of [rand(0, 2), rand(2.5, 4.5)]) {
    const r = rand(18, FIELD_R - 12);
    const sc = buildScarecrow();
    sc.position.set(Math.cos(a) * r, 0, Math.sin(a) * r);
    sc.rotation.y = rand(0, Math.PI * 2);
    scene.add(sc);
    obstacles.push({ x: sc.position.x, z: sc.position.z, r: 0.9 });
  }
...
// ---------------------------------------------------------------------------
// Farm buildings + dead trees
// ---------------------------------------------------------------------------

function buildFarm() {
  // barn off the north-east fence with one warm lit window
  ...
  barn.position.set(52, 0, -52);
  barn.rotation.y = Math.PI / 4 + Math.PI;
  scene.add(barn);

  // windmill (wind pump) to the west, slowly turning
  ...
  mill.position.set(-62, 0, -20);
  mill.rotation.y = -0.8;
  scene.add(mill);

  // dead trees ringing the field — black claws against the sky
  ...
  for (let i = 0; i < 11; i++) {
    ...
    // keep clear of the barn / windmill corners
    if (Math.hypot(Math.cos(a) * r - 52, Math.sin(a) * r + 52) < 16) continue;
    if (Math.hypot(Math.cos(a) * r + 62, Math.sin(a) * r + 20) < 14) continue;
