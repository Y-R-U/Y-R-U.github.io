...
// so far-away crows sound far away. The caw is two detuned saws swept down
// through a bandpass with a noise rasp on top.
...
export const AudioFX = {
  ...
  // Dusk wind: looped noise through a slow-wandering lowpass, plus a low
  // two-note drone underneath.
  startAmbient() {
  ...
  // Crow caw: raspy descending squawk. pitch < 1 = bigger bird.
  caw(pitch = 1, vol = 1) {
  ...
  // Flak cannon: sharp noise crack + low thump.
  shot() {
    if (!this.ctx) return;
    const t0 = this.ctx.currentTime;
    const src = this.ctx.createBufferSource();
    ...
    this.env(og, 0.07, 0.09, t0);
    o.connect(og).connect(this.master);
    o.start(t0);
    o.stop(t0 + 0.11);
  },

  // Crow killed: strangled squawk + feathery flutter.
  squawk(pitch = 1, vol = 1) {
    if (!this.ctx || vol < 0.05) return;
    const t0 = this.ctx.currentTime;
    const o = this.ctx.createOscillator();
    o.type = 'sawtooth';
    o.frequency.setValueAtTime(900 * pitch, t0);
    o.frequency.exponentialRampToValueAtTime(180 * pitch, t0 + 0.3);
    const f = this.ctx.createBiquadFilter();
    f.type = 'bandpass';
    f.frequency.value = 900 * pitch;
    f.Q.value = 2;
    const g = this.ctx.createGain();
    this.env(g, 0.18 * vol, 0.32, t0);
    o.connect(f).connect(g).connect(this.master);
    o.start(t0);
    o.stop(t0 + 0.35);
  ...
  boom(big, vol = 1) {
    ...
    const g = this.ctx.createGain();
    this.env(g, (big ? 0.55 : 0.28) * vol, dur, t0);
    src.connect(f).connect(g).connect(this.master);
    src.start(t0);
    src.stop(t0 + dur + 0.05);
    ...
    const og = this.ctx.createGain();
    this.env(og, (big ? 0.5 : 0.26) * vol, dur * 0.8, t0);
    o.connect(og).connect(this.master);
    o.start(t0);
    o.stop(t0 + dur);
  },
