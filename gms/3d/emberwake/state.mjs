...
export const fresh=()=>({version:3,coins:20,mastery:Object.fromEntries(PROFESSIONS.map(s=>[s,0])),region:'lab',name:'Alex',gender:'male',labStep:0,mainStage:0,relays:[],mainKills:[],ledger:false,reinforced:false,clues:[],speechQueue:[],lastSpeech:null,stage:0,hp:100,mana:100,x:0,z:18,weapon:0,crafted:false,magic:false,xp:Object.fromEntries(SKILLS.map(s=>[s,0])),bag:{wood:0,ore:0,raw:0,fish:3,embers:0},training:[0,0,0],kills:[],bossDead:false,beacon:false,belt:0,skills:0,muted:false});
...
export const fishHealing=s=>Math.min(100,45+5*(s.mastery?.Fishing||0));
...
export function validate(raw){
 const d=fresh();if(!raw||![1,2,3].includes(raw.version))return null;
