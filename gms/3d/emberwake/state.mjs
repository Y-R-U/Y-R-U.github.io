export const SKILLS=['Melee','Magic','Woodcutting','Mining','Fishing','Smithing','Cooking'];
export const PROFESSIONS=['Woodcutting','Mining','Fishing','Smithing','Cooking'];
export const threshold=level=>50*(level-1)*level;
export const level=xp=>Math.floor((1+Math.sqrt(1+4*xp/50))/2);
export const fresh=()=>({version:4,coins:20,bladeTier:0,tools:{Woodcutting:0,Mining:0,Fishing:0},mastery:Object.fromEntries(PROFESSIONS.map(s=>[s,0])),region:'lab',name:'Alex',gender:'male',labStep:0,mainStage:0,relays:[],mainKills:[],ledger:false,reinforced:false,clues:[],speechQueue:[],lastSpeech:null,stage:0,hp:100,mana:100,x:0,z:18,weapon:0,crafted:false,magic:false,xp:Object.fromEntries(SKILLS.map(s=>[s,0])),bag:{wood:0,ore:0,raw:0,fish:3,ingot:0,embers:0},training:[0,0,0],kills:[],bossDead:false,beacon:false,belt:0,skills:0,muted:false});
...
export const fishHealing=s=>Math.min(100,45+5*(s.mastery?.Fishing||0)+3*(s.mastery?.Cooking||0));
export function eat(s){if(s.bag.fish<1||s.hp>=100)return false;s.bag.fish--;s.hp=Math.min(100,s.hp+fishHealing(s));return true}
...
export function validate(raw){
 const d=fresh();if(!raw||![1,2,3,4].includes(raw.version))return null;
 // Add artisan fields before validating older schemas, preserving their earned ranks and XP.
 if(raw.version<4)raw={...raw,xp:{Cooking:0,...raw.xp},bag:{ingot:0,...raw.bag},mastery:{Smithing:0,Cooking:0,...raw.mastery},bladeTier:0,tools:{...d.tools}};
 ...
 const s={...d,...raw,version:4};
 if(raw.version<3){s.coins=20;s.mastery={...d.mastery};}
 if(!Number.isSafeInteger(s.coins)||s.coins<0||s.coins>1e9||!PROFESSIONS.every(k=>Number.isInteger(s.mastery?.[k])&&s.mastery[k]>=0&&s.mastery[k]<=Math.min(20,Math.floor(level(s.xp[k])/5))))return null;
 if(!Number.isInteger(s.bladeTier)||s.bladeTier<0||s.bladeTier>5||!Object.keys(d.tools).every(k=>Number.isInteger(s.tools?.[k])&&s.tools[k]>=0&&s.tools[k]<=3))return null;
 ...
 if(!Array.isArray(s.speechQueue)||!s.speechQueue.every(id=>typeof id==='string')||s.speechQueue.length>40)return null;
