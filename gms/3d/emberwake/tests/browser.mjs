...
await track();await wait(()=>emberwake.state.region==='island');assert.equal(await page.evaluate(()=>emberwake.world.player.userData.gender),'female');await close();console.log('lab escape complete');
...
await stage(7);await close();await track();await stage(8);await page.screenshot({path:'/private/tmp/emberwake-ending.png'});await close();await wait(()=>emberwake.state.region==='mainland');await close();await page.screenshot({path:'/private/tmp/emberwake-mainland-new.png'});
...
assert(bounds.every(b=>!b.overflow&&!b.clipped.length),JSON.stringify(bounds));await page.evaluate(()=>{const g=emberwake,o=g.world.objects.find(o=>o.id==='return');g.go(o.x,o.z,o)});await wait(()=>emberwake.state.region==='island');await track();await wait(()=>!document.querySelector('#dialogue').hidden);await close();await wait(()=>emberwake.state.region==='mainland');assert.equal(await page.evaluate(()=>emberwake.state.mainStage),6);
