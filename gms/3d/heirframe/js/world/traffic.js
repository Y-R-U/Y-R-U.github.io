...
export function buildTraffic(ctx) {
  const { scene, M, tier, updaters } = ctx;
  const R = rng(31);
  const lanes = [];
  ...
  const n = tier.traffic;
  const cars = [];
  for (let i = 0; i < n; i++) {
    const lane = lanes[i % lanes.length];
    cars.push({ lane, t: R(), off: (R() - 0.5) * 8, dy: (R() - 0.5) * 6, bob: R() * 6 });
  }
  ...
  const m4 = new THREE.Matrix4(), q = new THREE.Quaternion(), p = new THREE.Vector3(), s = new THREE.Vector3(1.4, 1.4, 1.4), e = new THREE.Euler();
  const place = (dt, time) => {
    for (let i = 0; i < n; i++) {
  ...
  const beam = new THREE.Mesh(new THREE.ExtrudeGeometry(beamShape, { steps: 300, extrudePath: curve, bevelEnabled: false }), M.skyStone);
  beam.layers.enable(REFLECT_LAYER); beam.castShadow = false;
  scene.add(beam);
  const glowTube = new THREE.Mesh(new THREE.TubeGeometry(curve, 300, 0.12, 4, true), glow);
  ...
  const NP = 36;
  const pylons = new THREE.InstancedMesh(pylonGeo, M.skyStone, NP);
  for (let i = 0; i < NP; i++) {
    const pt = curve.getPointAt(i / NP);
    m4.compose(new THREE.Vector3(pt.x, -10, pt.z), q.identity(), new THREE.Vector3(1, pt.y + 9.3, 1));
    pylons.setMatrixAt(i, m4);
  }
  pylons.layers.enable(REFLECT_LAYER);
  scene.add(pylons);
  ...
  updaters.push((dt) => {
    tt = (tt + dt * 22 / curve.getLength()) % 1;
    for (let i = 0; i < NC; i++) {
      const u = (tt - i * 12.2 / curve.getLength() + 1) % 1;
      curve.getPointAt(u, p); curve.getTangentAt(u, tan);
      p.y += 2.3;
      look.lookAt(new THREE.Vector3(), tan, up);
      q.setFromRotationMatrix(look).multiply(new THREE.Quaternion().setFromAxisAngle(up, Math.PI / 2));
      m4.compose(p, q, s.set(1, 1, 1));
      train.setMatrixAt(i, m4); wins.setMatrixAt(i, m4);
    }
