// One array-based predicate evaluator, shared by hotspot `if`, dialogue choices and anything else
// that gates on world state. Nothing is ever eval'd, so every predicate the linter reads is the
// one that runs. Lifted from FORGE with its ten campaign terms cut — this game has no schools,
// factions or standing to test.
...
// A quest's availability may never depend on how strong the player is. Nothing in Who Fights
// registers a `level: true` term yet; the check stays so the first one added is caught.
