...
class Editor {
  ...
  toggle(on = !this.on) {
    ...
    const player = window.__wf.player;
    if (player) {
  ...
  // Anything mirroring the document out of the browser — the dev hub's level tab writes it back
  // into data/levels/<id>.json. Fired on the settled change, not mid-drag.
  onChange(fn) { this.watchers.add(fn); return () => this.watchers.delete(fn); }

  notify() {
    for (const fn of this.watchers) {
      try { fn(this.doc); } catch (e) { console.error('[editor] watcher threw', e); }
    }
  }

  selectById(id) {
    const o = this.doc.objects.find(q => q.id === id);
    if (o) this.select(o);
    return !!o;
  }

  setObjectString(id, key, value) {
    const o = this.doc.objects.find(q => q.id === id);
    if (!o) return false;
    this.mutate(`s${id}.${key}`);
    o.p[key] = String(value ?? '').slice(0, 120);
    this.saveSoon();
    return true;
  }

  // Text is baked to a canvas texture, so it is rebuilt when the field is left rather than on
  // every keystroke. An `inside` board is drawn by doors.js, which is watching doc.rev already.
  rebuildObject(id) {
    const o = this.doc.objects.find(q => q.id === id);
    if (!o) return;
    if (this.selected && this.selected.id === id) this.refreshLive();
    else this.builder.district(o.dist);
    this.builder.refreshDecals(this.aoOpacity());
    this.saveNow();
    this.ui.sync();
  }
  ...
  setZone(zoneId) {
    if (this.selected) this.setZoneMany([this.selected.id], zoneId);
  }

  // Retoning a whole castle is one undo step and one rebuild per district, not nineteen of each.
  setZoneMany(ids, zoneId) {
    const want = new Set(ids);
    const hit = this.doc.objects.filter(o => want.has(o.id) && o.zone !== zoneId);
    if (!hit.length) return 0;
    this.mutate();
    const dists = new Set();
    for (const o of hit) { o.zone = zoneId; dists.add(o.dist); }
    for (const d of dists) this.builder.district(d);
    if (this.selected && want.has(this.selected.id)) this.refreshLive();
    this.builder.refreshDecals(this.aoOpacity());
    this.saveNow();
    this.ui.sync();
    return hit.length;
  ...
  groundPoint(e) {
    ...
    const hit = this.ray.intersectObjects(this.demo.terrain.chunks, false)[0];
