...
// The great hall's knobs. They belong to interior.js, but this is the module the quality
// registry is handed at boot, so this is where they get registered.
import { registerHallKnobs } from './interior.js';
...
#define FORGE_TAP(px,py) sum += texture2DCompare( shadowMap, sc.xy + vec2( (px)*rc.x - (py)*rc.y, (px)*rc.y + (py)*rc.x ) * f, sc.z );
	float wfPenumbra( sampler2D shadowMap, vec2 shadowMapSize, float grow, vec3 sc ) {
...
(function installPenumbra() {
  ...
  s = s.slice(0, a) +
    ...
    '\t\t\tshadow = wfPenumbra( shadowMap, shadowMapSize, shadowRadius, shadowCoord.xyz );\n' +
    s.slice(b);
...
export class Lighting {
  ...
  fitShadow(app, pad = 1) {
    ...
    // shadowRadius is the growth rate wfPenumbra reads: blur texels per unit of normalised
