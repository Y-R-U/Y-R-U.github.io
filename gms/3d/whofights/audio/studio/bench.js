...
// Lifted from ../forge/audio/ — the synthesis stack it drives is audio/js/{core,sfx}.js, the same
// two files js/game/audio.js fires in the game, so a sound tuned here is the sound that ships.
//
// `?from=<url>` is set when the dev hub opened this page; see the back control below.

import { createEngine } from '../js/core.js';
...
// Opened from the dev hub, the way back is the tab it was opened from: the opener still has the
// hub up on the music tab, so closing this one lands exactly where it left. A direct hit with a
// `from=` and no opener falls back to navigating there.
const from = new URLSearchParams(location.search).get('from');
const back = document.getElementById('back');
if (from) {
  back.textContent = '\u2039 Back to dev tools';
  back.classList.add('todev');
  back.href = from;
  back.addEventListener('click', e => {
    if (!window.opener || window.opener.closed) return;
    e.preventDefault();
    try { window.opener.focus(); } catch { /* another origin, or gone */ }
    window.close();
  });
}

window.__lab = { ctx, eng, SFX, SFX_IDS, store, state, from, report: () => report(state, store, 'keep'), ready: true };
