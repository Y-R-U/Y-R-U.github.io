export async function createAudio(){
 ...
 let settings={music:.45,sfx:.6};try{const stored=JSON.parse(localStorage.getItem('tinpot.audio'));if(stored)for(const k of ['music','sfx'])if(Number.isFinite(stored[k]))settings[k]=Math.max(0,Math.min(1,stored[k]));}catch{}
 ...
 return {change,reset(){lastEvent=0;},settings(){return {...settings};},set(key,value){settings[key]=Math.max(0,Math.min(1,value));try{localStorage.setItem('tinpot.audio',JSON.stringify(settings));}catch{}},update(w,dt,paused){for(let i=voices.length-1;i>=0;i--){const v=voices[i];v.gain+=(v.target-v.gain)*Math.min(1,dt*3);v.el.volume=Math.max(0,Math.min(1,v.gain*settings.music*(paused?.35:1)));if(v.target===0&&v.gain<.005){v.el.pause();voices.splice(i,1);}}for(const e of w.events){if(e.id<=lastEvent)continue;lastEvent=e.id;if(!paused&&w.time-e.time<.2&&['shot','death','explosion'].includes(e.type))effect(e.type);}fireTimer-=dt;if(fireTimer<0&&!paused&&w.trees.some(t=>t.burn>0)){fireTimer=.5;effect('fire');}},snapshot(){return {cue,unlocked,voices:voices.map(v=>({file:v.el.currentSrc,time:v.el.currentTime,paused:v.el.paused,volume:v.el.volume,ready:v.el.readyState})),sfxCount,settings:{...settings}};}};
}
