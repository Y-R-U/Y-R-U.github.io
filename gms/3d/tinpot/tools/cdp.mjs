...
export const BASE=process.env.BASE||'http://127.0.0.1:8888/gms/3d/tinpot/';
...
export async function connect(){
  ...
  function send(method,params={},timeout){timeout=timeout??20000;return new Promise((resolve,reject)=>{const key=++id,timer=setTimeout(()=>{pending.delete(key);reject(new Error('CDP timeout: '+method));},timeout);pending.set(key,{resolve,reject,timer});ws.send(JSON.stringify({id:key,method,params}));});}
  ...
  const api={send,errors,requests,on(method,fn){const list=listeners.get(method)||[];list.push(fn);listeners.set(method,list);},async eval(expression,timeout){const r=await send('Runtime.evaluate',{expression,returnByValue:true,awaitPromise:true},timeout);if(r.exceptionDetails)throw new Error(r.exceptionDetails.exception?.description||r.exceptionDetails.text);return r.result.value;},
    async goto(url=BASE+'?test=1',metrics={width:1366,height:768,deviceScaleFactor:1,mobile:false}){await send('Page.enable');await send('Runtime.enable');await send('Log.enable');await send('Network.enable');await send('Network.setCacheDisabled',{cacheDisabled:true});await send('Emulation.setDeviceMetricsOverride',metrics);await api.eval('window.__TINPOT_BOOTED__=false;delete window.__TINPOT_FAILED__');return send('Page.navigate',{url});},
    ...
    async shot(path,timeout){await api.eval('new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)))',timeout);const r=await send('Page.captureScreenshot',{format:'png'},timeout);await mkdir(dirname(path),{recursive:true});await writeFile(path,Buffer.from(r.data,'base64'));return r.data;},
