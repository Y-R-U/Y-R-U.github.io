...
const PAD_BY_TYPE = { tower: 0.15, wallRun: 0.15, house: 0.25, mass: 0.25 };
...
export class Colliders {
  ...
  rebuildWalk(doc) {
    ...
    for (const o of doc.objects) {
      const [hw, hd] = TYPES[o.type].plan(o.p);
      const r = this.terrain.range(o.x, o.z, hw, hd, o.ry);
      put(o.x, o.z, hw + 0.12, hd + 0.12, o.ry, r.lo - 2, r.hi + tall(o), 0);
    }

    for (const d of doc.districts) {
      if (d.bridge) {
        const { x, z, halfSpan } = d.bridge;
        const deck = waterY(x) + 1.55 + 0.16;
        const half = halfSpan + 3.5 + 0.2;
        put(x, z, 3.0, half, 0, deck - 0.75, deck, WALK.stepUp);
        for (const s of [-1, 1]) put(x + s * 3.4, z, 0.31, half, 0, deck - 0.2, deck + 0.77, 0);
      }
      ...
      for (const k of d.kerbs) {
        ...
        if (k.top - ry < 0.4) continue;
        put(k.x, k.z, 0.42, k.len / 2, 0, ry - 0.3, k.top + 0.13, 1.5);
      }
    }
