...
//
//   node tools/shot.mjs --shot=nebula_back --preset=high --w=1600 --h=900
//   node tools/shot.mjs --all --preset=medium
//   node tools/shot.mjs --shot=nebula_back --perf --headed   ← real GPU, for the budget gate
...
async function chrome() {
  const flags = [
    ...
    `--user-data-dir=/tmp/mono-cdp-${process.pid}`,
    `--window-size=${W},${H}`,
...
async function main() {
  ...
  const shots = args.all ? await listScenarios(S) : (args.shot ? [args.shot] : []);
  if (!shots.length) {
    console.log(args.all ? 'no scenarios registered' : 'need --shot=<id> or --all');
  ...
  for (const shot of shots) {
    ...
    await waitFor(S, `window.__mono && window.__mono.ready`, 15000);
    ...
    const stats = await evalJSON(S, `window.__mono.stats()`);
    const meta = await evalJSON(S, `window.__mono.scenarios.find(s=>s.id===${JSON.stringify(shot)}) || null`);
    ...
    writeFileSync(png.replace(/\.png$/, '.json'), JSON.stringify({
      shot, ref: meta?.ref, preset: PRESET, dpr: +DPR, w: W, h: H, headed: HEADED, stats,
    }, null, 2));
    ...
    console.log(`${shot}  ${stats.fps.toFixed(0)}fps  gpu ${fmt(stats.gpuP95)}ms  cpu ${fmt(stats.cpuP95)}ms  ${stats.calls} calls  ${(stats.tris / 1000).toFixed(0)}k tris  ${stats.texMB.toFixed(1)}MB tex  → ${png}`);
  }
...
async function settle(S, frames) {
  const start = await evalJSON(S, `window.__mono.frames()`);
  const t0 = Date.now();
  while (Date.now() - t0 < 20000) {
    const n = await evalJSON(S, `window.__mono.frames()`);
...
async function listScenarios(S) {
  ...
  await waitFor(S, `window.__mono && window.__mono.ready`, 15000);
  return await evalJSON(S, `window.__mono.scenarios.map(s=>s.id)`);
}

// A run that throws used to leave its browser and its profile dir behind. Enough of those and the
// machine is loaded enough to make every timing on it meaningless.
let PROC = null;
function cleanup(proc) {
  const dir = `/tmp/mono-cdp-${process.pid}`;
