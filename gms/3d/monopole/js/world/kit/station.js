...
function breakUp(m) {
  m.onBeforeCompile = sh => {
    ...
    sh.fragmentShader = `varying vec3 vSP; varying vec3 vSN;
      uniform float uSRough, uSPlane, uSPanel, uSDirt, uSpillPow;
  ...
  m.customProgramCacheKey = () => 'stationbreak3';
  return m;
}
...
const MODULES = {
  ...
  bay(g, R, p) {
    ...
    // plates set into it, so the plates are narrow and a dark coaming caps each outer edge. The
    // hot line down each plate edge is 8500_06's own trick: the containers are separated by orange
    // slot lights, not by shadow, so the run stays legible at thumbnail size.
    for (const s of [-1, 1]) {
      ...
      g.dark.push(box(W * 0.34, 1.9, D * 0.94, s * W * 0.26, H / 2 + 0.6, D / 2, 0, 0, 0, 0.3));
      g.dark.push(box(W * 0.13, 2.6, D * 0.94, s * W * 0.435, H / 2 + 0.7, D / 2, 0, 0, 0, 0.34));
      for (const e of [-1, 1]) {
        g.strip.push(box(0.5, 0.30, D * 0.86, s * W * 0.26 + e * W * 0.155, H / 2 + 1.55, D / 2));
      }
    }
    ...
    if (open < 0.34) {
      ...
      g.glow.push(glowQuad(mw * 0.90, mh * 0.40, '-z', 0, -mh * 0.10, -0.2, warm, 0.30 + 0.34 * open, 1.5));
      g.glow.push(glowQuad(mw * 0.72, 0.85, '-z', 0, mh * 0.26, -0.25, warm, 0.9 + 0.9 * open, 1.1));
