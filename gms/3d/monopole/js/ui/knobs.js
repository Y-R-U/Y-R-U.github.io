...
// Shown up front; everything else goes one tap deeper. A phone cannot present them all.
const PRIMARY = ['exposure', 'nebGain', 'nebRays', 'keyPower', 'fillPower'];

let rebuild = null;
export const refreshKnobs = () => rebuild?.();

export function buildKnobs(app) {
  ...
  rebuild = () => {
    ...
    let html = `<div class="grp"><h4>Preset</h4><div class="presets">${presets}</div></div>`;
  ...
  // Safari does not fire `input` on <select>, so a phone user's change silently did nothing.
...
function fmtVal(v) { return typeof v === 'number' ? (Number.isInteger(v) ? v : +v.toFixed(4)) : v; }
